...
- **Orthanc authorization plugin configuration:** The plugin's callback contract needs to be verified against the actual orthanc-authorization plugin docs to confirm the exact request shape NestJS should expect. The current design assumes Orthanc forwards the full Authorization header and request URI in a JSON POST body, but the plugin's actual payload format may differ.

- **Cornerstone3D WADO-RS URL configuration:** Need to confirm that Cornerstone3D's ImageLoader can be pointed at a non-standard WADO-RS base URL (the ALB path prefix `/dicom-web/wado-rs/`) without custom loader registration. If the loader assumes a root-relative DICOMweb path, a thin adapter or URL rewrite may be needed.

- **Refresh token storage on the client:** The design assumes refresh tokens are stored client-side but does not specify the storage mechanism. httpOnly cookies prevent XSS-based token theft but require CSRF protection. localStorage is vulnerable to XSS but simpler to implement. This decision has security implications that should be finalized before implementation.
